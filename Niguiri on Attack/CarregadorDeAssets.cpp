	...
	if (arq_recursos) {
		...
		while (!arq_recursos.eof()) {
			...
			if (!arq_recursos.fail()) {
				...
				if (tipo_asset == "fonte") {
					arq_recursos >> caminho_asset >> tamanho_fonte;
					gDebug.depurar("caminho_asset", caminho_asset);
					gDebug.depurar("tamanho_fonte", tamanho_fonte);
					gRecursos.carregarFonte(nome_asset, caminho_asset, stoi(tamanho_fonte, &sz));
				}
			}
		}
		return true;
